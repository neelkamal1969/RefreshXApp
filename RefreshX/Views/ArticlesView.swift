...
struct ArticlesView: View {
    ...
    var body: some View {
        NavigationView {
            ZStack {
                Color(.systemBackground) // White background as requested
                    .edgesIgnoringSafeArea(.all)
                ...
                VStack(spacing: 0) {
                    ...
                    // Filter and Sort inline with section title
                    HStack {
                        Text("Latest Articles")
                            .font(.title3)
                            .fontWeight(.bold)
                        ...
                        Menu {
                            ...
                            Menu {
                                ...
                                ForEach(FocusArea.allCases, id: \.self) { focusArea in
                                    Button(focusArea.rawValue.capitalized) {
                                        viewModel.selectedFocusArea = focusArea
                                        showToastFor("Showing \(focusArea.rawValue) articles")
                                    }
                                }
                            } label: {
                            ...
                            Button(action: {
                                ...
                                Label("Read Articles", systemImage: showReadOnly ? "book.fill" : "book")
                            }
                            ...
                            Menu {
                                ForEach(SortOption.allCases) { option in
                                    Button(action: {
                                        ...
                                        showToastFor("Sorted by \(option.rawValue)")
                                    }) {
                                        HStack {
                    ...
                    ScrollView {
                        VStack(spacing: 0) {
                            ...
                            // Section Title
                            if searchText.isEmpty && !showFavoritesOnly && !showReadOnly && !viewModel.filteredArticles.isEmpty {
                                HStack {
                                    Text("All Articles")
                                        .font(.title3)
                                        .fontWeight(.bold)
                                ...
                                .padding(.top, 16)
                                .padding(.bottom, 8)
                            } else if !filteredSortedArticles.isEmpty {
                                HStack {
                                    Text(sectionTitle)
                                        .font(.title3)
                                        .fontWeight(.bold)
    ...
    // Active filters bar with clear buttons
    private var activeFiltersBar: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                if viewModel.selectedFocusArea != nil {
                ...
                // Clear all filters button
                if viewModel.selectedFocusArea != nil || showFavoritesOnly || showReadOnly {
                    Button(action: {
                        withAnimation {
                            viewModel.selectedFocusArea = nil
                            showFavoritesOnly = false
    ...
    private var searchBar: some View {
        ...
        .padding(.vertical, 8)
        .background(Color(.systemGray6)) // Light gray background as requested
        .cornerRadius(10)
    }
        ...
        return LazyVGrid(columns: columns, spacing: 16) {
            ForEach(filteredSortedArticles) { article in
                NavigationLink(destination: ArticleDetailView(
                    ...
                    .frame(height: 210) // Fixed height for consistency
                }
                .buttonStyle(PlainButtonStyle())
            }
        }
    ...
    private func typeColor(for focusArea: FocusArea) -> Color {
        switch focusArea {
        case .eye:
            return .blue // Blue as requested
        case .back:
            return .green // Green as requested
        case .wrist:
            return .orange // Orange as requested
        }
    }
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            ZStack(alignment: .bottomLeading) {
                ...
                Text(article.type.rawValue.capitalized)
                    ...
                    .foregroundColor(typeColor(for: article.type))
                    .cornerRadius(8)
                    .padding(12)
            }
            .overlay(alignment: .topTrailing) {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            ZStack(alignment: .bottomLeading) {
                ...
                // Status indicators overlay
                HStack(spacing: 4) {
                    // Category pill with correct color
                    Text(article.type.rawValue.capitalized)
                        .font(.caption2)
                        .fontWeight(.semibold)
                        .padding(.horizontal, 8)
            ...
            .padding(10)
            .frame(height: 70) // Fixed content height
        }
        .background(Color(.systemBackground))
        .cornerRadius(12)
        .shadow(color: Color.black.opacity(0.1), radius: 3, x: 0, y: 2)
